...
dotnet restore "N8s IP Scanner.csproj" --configfile "NuGet.Config" --ignore-failed-sources -v minimal
if errorlevel 1 goto fail

echo.
echo Building release...
dotnet build "N8s IP Scanner.csproj" -c Release --no-restore
if errorlevel 1 goto fail

echo.
echo Publishing framework-dependent Windows app...
dotnet publish "N8s IP Scanner.csproj" -c Release --no-restore --self-contained false -p:PublishSingleFile=false -o "%~dp0dist"
if errorlevel 1 goto fail

echo.
echo Done.
echo EXE created here:
echo %~dp0dist\N8s IP Scanner.exe
echo.
